...
def create_table():
	...
	elidart.execute("CREATE TABLE IF NOT EXISTS messages(id integer PRIMARY KEY, message text)")
	con.commit()
	con.close()
	return
...
def save_messages(msg): #guarda los mensajes en la tabla
	con = sqlite3.connect('users.db', check_same_thread=False)
	elidart = con.cursor()
	query=  f"INSERT INTO messages(message) values('{msg}')"
	elidart.execute(query)
	con.commit()
	con.close()
	return
